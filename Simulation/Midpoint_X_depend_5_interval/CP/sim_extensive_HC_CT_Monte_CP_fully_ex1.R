...
interval   <- c(0,12,24,36,48)
Monte_out  <- list()
M          <- 500

for(m in 1:M){

  full_dt <- gen_interval_censor_HC_CT(
    nH             = 1800,
    nC             = 600,
    est            = c(1,-1,0.5,-0.5,0,0,1,-1,0.5),
    delta          = rep(0,9),
    seed           = m*153 + 3,
    interval       = c(0),
    gamma          = 0,
    lambda0        = c(exp(1)),
    delta_lambda   = c(exp(0)),
    ct_intervals   = ct_interval,
    ct_margin      = ct_margin,
    ct_multi       = 1,
    ct_event       = 0.1,
    hc_intervals   = hc_interval,
    hc_margin      = hc_margin,
    hc_multi       = 1,
    hc_event       = 0.1,
    kappa          = c(0),
  ...
  kappa[K+1] <- max(full_dt$obs_L, full_dt$obs_R, full_dt$time, na.rm = TRUE) + 0.0001

  full_dt$L_int <- findInterval(full_dt$obs_L, kappa)
  full_dt$R_int <- findInterval(full_dt$obs_R, kappa)

  out.H <- full_dt[full_dt$HC == 1, ]
  out.C <- full_dt[full_dt$HC == 0, ]
  ...
  model.data <- list(
    "idxL", "idxI", "idxR", "idxE",
    "lower.int.obs", "upper.int.obs",
    "X", "p", "nint", "kappa", "R", "L", "ones",
    "idxL.C", "idxI.C", "idxR.C", "idxE.C",
    "lower.int.obs.C", "upper.int.obs.C",
    "X.C", "R.C", "L.C", "ones.C",
    "idxcluster", "n", "n.C", "trt.C"
  )

  model.parameters <- c("beta", "alpha", "nu", "betaC", "alphaC", "gammaC", "zz", "cc", "aaa1")
  model.fit.cluster <- jags(
    model.file  = "exact_likelihood_random_effect_HC_CT.txt",
    data        = model.data,
  ...
  saveRDS(Monte_out, file = "fully_ex1(3).rds")
}