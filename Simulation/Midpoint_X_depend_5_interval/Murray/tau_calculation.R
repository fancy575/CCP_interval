setwd("/scratch/g/skim/CT_HD_Bayesian_ones(1228)/Midpoint_X_depend_5_interval/Murray")
...
tau_dist_fun <- function(indata){
  ...
  for(m in 1:length(indata)){
    tau_dist  <- matrix(nrow=nrow(indata[[m]]$BUGSoutput$sims.list$t),
                        ncol=ncol(indata[[m]]$BUGSoutput$sims.list$t))
    pos_alpha1 <- 10
    for(i in 1:ncol(tau_dist)){
      if(i < pos_alpha1){
        tau_dist[,i] <- ifelse(indata[[m]]$BUGSoutput$sims.list$t[,i] == 1,
                               indata[[m]]$BUGSoutput$sims.list$mmm, 
                               200)
      }
      else if(i==pos_alpha1){
        tau_dist[,i] <- ifelse(indata[[m]]$BUGSoutput$sims.list$t[,i] == 1,
                               0.0001, 
                               200)
      }else{
        tau_dist[,i] <- ifelse(indata[[m]]$BUGSoutput$sims.list$t[,i] == 1,
                               indata[[m]]$BUGSoutput$sims.list$mmm1, 
                               200)
      }
  ...
  tau_final <- cbind(tau_25,tau_50,tau_75)
  rownames(tau_final) <- c(paste("X",1:9,sep=""),paste("alpha",1:5))
  return(tau_final)
}
