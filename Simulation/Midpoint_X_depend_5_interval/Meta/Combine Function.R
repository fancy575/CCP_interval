setwd("/scratch/g/skim/CT_HD_Bayesian_ones(1228)/Midpoint_X_depend_5_interval/Meta/combine")
...
summary_func <- function(rds_data,true_value){
  ...
  raw_result <- raw_dt[,c(1:28,30:59)]
  raw_ci25 <- raw_25[,c(1:28,30:59)]
  raw_ci75 <- raw_75[,c(1:28,30:59)]
  ...
  raw_sum <- data.frame(true=c(rep(true_value[1],5),rep(true_value[2],5), true_value[3:21],rep(NA,5),rep(NA,9),true_value[22],rep(NA,14)),
                        mean = apply(raw_result, 2, mean),
                        sd=apply(raw_result,2,sd))
  
  raw_mse <- sweep(raw_result[,c(1:29,44)],2,
                   c(rep(true_value[1],5),rep(true_value[2],5), true_value[3:21],true_value[22]),"-")
  raw_se_margin <-apply(raw_result[,c(1:29,44)],2,sd)/sqrt(length(rds_data))*qnorm(0.975)
  
  
  raw_mse <- raw_mse^2
  raw_mse <- apply(raw_mse, 2, mean)
  
  raw_sum$mse <- c(raw_mse[c(1:29)],rep(NA,5),rep(NA,9),raw_mse[30],rep(NA,14))
  raw_sum$se_margin <- c(raw_se_margin[1:29],rep(NA,5),rep(NA,9),raw_se_margin[30],rep(NA,14))
  
  true_mt <- t(replicate(length(rds_data)-length(remove_id),c(rep(true_value[1],5),rep(true_value[2],5), true_value[3:21],true_value[22])))
  ci <- ifelse(raw_ci25[,c(1:29,44)] <= true_mt & true_mt <= raw_ci75[,c(1:29,44)],1,0 )
  
  raw_sum$cr <- c(apply(ci[,1:29],2,mean),rep(NA,14),mean(ci[,30]),rep(NA,14))
  
  
  raw_sum <- raw_sum[c(1:10,29,11:28,44,30:43,45:58),]
  raw_sum[31:58,c(1,3,4,5)] <- ""
