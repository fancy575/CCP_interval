...
for(m in 1:M){
  ...
  full_dt <- gen_interval_censor_HC_CT(
    ...
    delta          = c(rep(0,5),rep(-0.2,4)),
    seed           = m*19 + 4,
    interval       = c(0),
    gamma          = 0,
    lambda0        = c(exp(2.4)),
    delta_lambda   = c(exp(-0.2)),
  ...
  model.parameters <- model.parameters <- c("beta","alpha","nu","betaC","alphaC","gammaC","tau_sb","tau_sa","mu_sb","mu_sa")  #Meta method
  
  model.fit.cluster <- jags(model.file= "exact_likelihood_random_effect_shrinkdiff.txt", data=model.data,
                            parameters=model.parameters, n.chains = 2,n.iter=5000,n.thin=5,n.burnin=1000)

  Monte_out[[m]] <- model.fit.cluster
  saveRDS(Monte_out, file = "nonfully2.4_02n(4).rds")
}