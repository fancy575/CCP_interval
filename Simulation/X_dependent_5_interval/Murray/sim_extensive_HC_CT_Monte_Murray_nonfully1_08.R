...
for(m in 1:M){
  ...
  full_dt <- gen_interval_censor_HC_CT(
    ...
    delta          = c(rep(0,5),rep(0.8,4)),
    seed           = m*587 + 5,
    interval       = c(0),
    gamma          = 0,
    lambda0        = c(exp(1)),
    delta_lambda   = c(exp(0.8)),
  ...
  model.parameters <- c("beta","alpha","nu","betaC","alphaC","gammaC","t","mmm","mmm1") # Murray's Method
  ...
  saveRDS(Monte_out, file = "nonfully1_08(5).rds")
}